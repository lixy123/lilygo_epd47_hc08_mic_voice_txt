...
//开机时如果按住此键可进入设置模式
#define BUTTON_BOOT_PRESS  35

#define BUTTON_SLEEP_PIN  39
#define BUTTON_CLEAR_SCREEN_PIN  34
#define uS_TO_S_FACTOR 1000000  /* Conversion factor for micro seconds to seconds */

Button2 BUTTON_SLEEP_BTN = Button2(BUTTON_SLEEP_PIN);
Button2 BUTTON_CLEAR_SCREEN_BTN = Button2(BUTTON_CLEAR_SCREEN_PIN);
...
//编译环境:
//    1.Arduino 1.8.13
//    2.扩展板引用地址配置成: https://raw.githubusercontent.com/espressif/arduino-esp32/gh-pages/package_esp32_dev_index.json
//    3.安装： 安装esp32的官方开发包 esp32 by Espressif Systems 版本 1.05-rc6
//    4.开发板选择: TTGO T-WATCH
//    5.选择ESP32连接的端口号，编译，上传烧写固件
//    编译大小约 3.3M
//
//使用说明：
//  ################# ESP32 #################################
...
void tap(Button2& btn) {
  if (btn == BUTTON_SLEEP_BTN)
  {
    goto_sleep();
  }
  else if (btn == BUTTON_CLEAR_SCREEN_BTN)
  {
...
bool readparams()
{
  ...
  //如果report_address这个值还没有，说明本机器没有配置默认
  if (report_address == "")
  {
...
bool wait_loud()
{
  ...
  while (true)
  {
    ...
    BUTTON_SLEEP_BTN.loop();
    BUTTON_CLEAR_SCREEN_BTN.loop();
...
void setupMode() {
  ...
  Show_hz("用手机或电脑连接AP ESP32SETUP, 访问http://192.168.4.1 设置墨水屏参数", false);
}
...
void init_hard()
{
  ...
  //SPIFFS.begin(true）在本机首次运行，如果spiff未格式化
  //会报错， 然后需要一点时间(5-10秒）进行初始化，最终会成功，并下一步
  //E (1025) SPIFFS: mount failed, -10025 ?
...
void setup()
{
  ...
  BUTTON_SLEEP_BTN.setTapHandler(tap);
  BUTTON_CLEAR_SCREEN_BTN.setTapHandler(tap);

  //初始化配置类
  preferences.begin("wifi-config");
  readparams();

  //pdm mic用15倍即可，不需要按配置参数
  volume_double = "15";
